<?php
    ...
    if(verificar_datos("[a-zA-Z0-9·ÈÌÛ˙¡…Õ”⁄Ò—().,$#\-\/ ]{1,70}",$nombre)){
        echo '
            <div class="notification is-danger is-light">
                <strong>°Ocurrio un error inesperado!</strong><br>
    ...
    if($check_categoria->rowCount()<=0){
        echo '
            <div class="notification is-danger is-light">
                <strong>°Ocurrio un error inesperado!</strong><br>
                La categorÌa seleccionada no existe
            </div>
        ';
        exit();
    }
	...
	if($_FILES['producto_foto']['name']!="" && $_FILES['producto_foto']['size']>0){
		...
		if(mime_content_type($_FILES['producto_foto']['tmp_name'])!="image/jpeg" && mime_content_type($_FILES['producto_foto']['tmp_name'])!="image/png"){
			echo '
	            <div class="notification is-danger is-light">
	                <strong>°Ocurrio un error inesperado!</strong><br>
	                La imagen que ha seleccionado es de un formato que no est· permitido
	            </div>
	        ';
	        exit();
		}
		...
		if(($_FILES['producto_foto']['size']/1024)>3072){
			echo '
	            <div class="notification is-danger is-light">
	                <strong>°Ocurrio un error inesperado!</strong><br>
	                La imagen que ha seleccionado supera el lÌmite de peso permitido
	            </div>
	        ';
			exit();
		}
